...
const MENU_ITEMS = [
    ...
    { id: 'manual', icon: 'manual', label: 'คู่มือการใช้งาน', active: false, href: '#' },
    {
        ...
        submenu: [
            ...
            { id: 'agent-memory', label: 'Agent Memory Console', href: '/agent-memory', adminOnly: true }
        ]
    }
];
...
export default function Navbar({ activePage = 'profile' }) {
    ...
    const isAdmin = mounted && normalizeUserCode(user?.usercode) === normalizeUserCode(ADMIN_USER_ID);
                                                ...
                                                <div className="absolute top-full left-0 pt-2 hidden group-hover:block min-w-[200px] z-50">
                                                    ...
                                                    >
                                                        {item.submenu.map((subItem) => {
                                                            ...
                                                            const hiddenForGuest = isGuest && isPortfolioSettings;
                                                            const hiddenForNonAdmin = subItem.adminOnly && !isAdmin;

                                                            if (hiddenForGuest || hiddenForNonAdmin) {
                                                                return null;
                                                            }
                                                            ...
                                                            return (
                                                                ...
                                                                >
                                                                    <span className="flex items-center gap-2">
                                                                        {subItem.label}
                                                                        {subItem.adminOnly ? (
                                                                            <span className="rounded-full border border-orange-400/20 bg-orange-500/10 px-2 py-0.5 text-[10px] uppercase tracking-wide text-orange-200">
                                                                                admin
                                                                            </span>
                                                                        ) : null}
                                                                    </span>
