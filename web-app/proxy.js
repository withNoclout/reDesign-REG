...
const PUBLIC_PATHS = new Set(['/', '/share']);
const GUEST_SHARE_PATHS = new Set([
    '/main',
    '/share',
    '/grade',
    '/grade/schedule',
...
export function proxy(request) {
    ...
    const hasShareToken = Boolean(request.nextUrl.searchParams.get('t'));
    ...
    if (!isAssetOrApi) {
        ...
        const isGuestSharePath = hasShareToken && (GUEST_SHARE_PATHS.has(pathname) || pathname.startsWith('/evaluation/form/'));

        if (pathname === '/' && hasAuthCookie && !sessionExpired) {
        ...
        if (!isPublicPath && !isGuestSharePath && !hasAuthCookie) {
    ...
    if (sessionExpired) {
        response.cookies.set('reg_token', '', { path: '/', maxAge: 0 });
        response.cookies.set('reg_identity', '', { path: '/', maxAge: 0 });
        response.cookies.set('reg_sso_session', '', { path: '/', maxAge: 0 });
    }
